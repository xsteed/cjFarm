...
/** 状态文案直接复用 api 里的枚举，避免纸质单据与列表显示不一致 */
...
function itemRows(order, withPrice) {
...
/**
 * 生成 A4 单据 HTML。
 * @param {object} order  订单详情（getOrder 返回的完整对象）
 * @param {'kitchen'|'guest'} docType
 * @param {object} opt  shopName 店铺名 / reprint 是否补打
 */
export function buildA4Html(order, docType, opt = {}) {
  const meta = DOC_META[docType] || DOC_META.guest
  const withPrice = meta.withPrice
  const cols = withPrice ? 4 : 2
  const headCols = withPrice ? '<tr><th>菜品</th><th class="c">单价</th><th class="c">数量</th><th class="c">小计</th></tr>' : '<tr><th>菜品</th><th class="c">数量</th></tr>'
  ...
  const sumRows = withPrice
    ? `<div class="sum"><span>菜品金额</span><b>¥${money(order.dishAmount)}</b></div>
       ...
       ${order.discountAmount > 0 ? `<div class="sum"><span>优惠</span><b>-¥${money(order.discountAmount)}</b></div>` : ''}
  ...
  const notesHtml = notes.length
    ? `<div class="notes">${notes.map((n) => `<div>${esc(n)}</div>`).join('')}</div>`
    : ''
  ...
  <section class="sheet">
    ...
    <div class="meta">
      <div class="row">
        ...
        <div><span class="k">状态</span>${esc(ORDER_STATUS[order.orderStatus]?.label || order.orderStatus || '-')}</div>
      </div>
      <div class="row">
...
async function resolveShopName(api) {
 ...
 * @param {'kitchen'|'guest'} docType
 * @param {object} opt { getPublicConfig, reprint }
 * @returns {Promise<boolean>} 是否被浏览器拦截
 */
export async function printOrderA4(order, docType, opt = {}) {
