 ...
 */
export async function renderQrCanvas(text, opts = {}) {
...
/** 生成二维码 PNG dataURL。 */
export async function renderQrDataUrl(text, opts = {}) {
...
function cardInner(card, o) {
  ...
  if (o.showShop && o.shopName) {
    lines.push(`<div style="font-size:3.1mm;font-weight:600;color:#F0481F;letter-spacing:.2mm">${esc(o.shopName)}</div>`)
  }
  if (o.showTableName && card.tableName) {
    lines.push(`<div style="font-size:4.4mm;font-weight:700;color:#1f2329;line-height:1.2">${esc(card.tableName)}</div>`)
  }
 ...
 * @param {Array<{tableNo,tableName,dataUrl,url}>} cards
 * @param {object} opts mode(sheet|label) / columns / cardW / cardH / gap / showShop / shopName / showTableName / showTableNo / hint / cutLine
 */
export function buildPrintSheetHtml(cards, opts = {}) {
  const o = {
    mode: 'sheet',
    columns: 3,
    cardW: 60,
  ...
  const pageHtml = pages
    .map((items, pi) => {
      ...
      const head =
        o.mode === 'label'
          ? ''
          : `<div class="phead"><span>${esc(o.shopName || '')}　每页 ${cols} × ${rows}</span><span>第 ${pi + 1} / ${pages.length} 页　共 ${cards.length} 张</span></div>`
  ...
  return `<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="utf-8"><title>桌台点餐码 ${
    o.shopName ? '· ' + esc(o.shopName) : ''
  }</title>
<style>
