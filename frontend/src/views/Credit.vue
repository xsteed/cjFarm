<template>
  <div>
    ...
    <div
      class="page-card"
      :style="canReport ? 'margin-top: 16px' : ''"
    >
      <div class="toolbar">
      ...
      >
        ...
        >
          <div class="mcard-hd">
            ...
            <t-tag
              :theme="row.creditStatus === 1 ? 'warning' : 'success'"
              variant="light"
            >
          ...
          <div class="mcard-ft">
            <t-button
              v-if="row.creditStatus === 1 && canSettle"
              theme="primary"
              variant="text"
              size="small"
              @click="onSettle(row)"
      ...
      <div v-if="detail">
        ...
        >
          ...
          <t-descriptions-item label="状态">
            {{ detail.creditStatus === 1 ? '待收款' : '已结清' }}
          </t-descriptions-item>
          <t-descriptions-item label="挂账时间">
            {{ detail.settleTime || detail.createTime }}
          </t-descriptions-item>
          <t-descriptions-item label="操作人">
            {{ detail.settleOperator || '-' }}
          </t-descriptions-item>
          <t-descriptions-item label="挂账人/备注">
            {{ detail.settleRemark || '-' }}
          </t-descriptions-item>
          <t-descriptions-item label="核销时间">
            {{ detail.creditSettleTime || '-' }}
          </t-descriptions-item>
        </t-descriptions>
        <t-table
          :data="detail.items"
        ...
        <div class="amount-summary">
          <div>
            应收金额：<span class="money">¥{{ $money(detail.totalAmount) }}</span>
          </div>
          <div class="total">
            实收金额：<span class="money">¥{{ $money(detail.paidAmount) }}</span>
          </div>
        </div>
      </div>
    </t-dialog>
...
import type { OrderDetail, OrderSummary, ReportSummary } from '../types/entities';
...
const tab = ref<RadioValue>('pending');
const keyword = ref('');
...
const form = reactive<SettleForm>({ orderId: 0, amount: 0, payType: '现金', remark: '' });
const payTypes: string[] = ['现金', '微信', '支付宝', '银行卡', '其他'];
...
async function load(): Promise<void> {
  loading.value = true;
  try {
    const params: PageQuery = { pageNum: pagination.current, pageSize: pagination.pageSize, settleType: 'credit' };
    ...
    list.value = kw ? rows.filter(r => (r.settleRemark || '').includes(kw) || (r.orderNo || '').includes(kw)) : rows;
    // 前端过滤后,分页总数必须跟着列表走:沿用服务端总数会让页数虚高,翻到没有数据的空页。
    pagination.total = kw ? list.value.length : res.total;
...
function onSettle(row: OrderSummary): void {
...
async function openDetail(row: OrderSummary): Promise<void> {
  try {
    detail.value = await getOrder(row.orderId as number);
