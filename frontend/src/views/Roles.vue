<template>
  <div class="page-card">
    ...
    <div v-if="isMobile" class="m-list">
      <div v-if="!list.length" class="m-empty">{{ loading ? '加载中…' : '暂无角色' }}</div>
      <div v-for="row in list" :key="row.roleId" class="mcard">
        <div class="mcard-hd">
          <span class="mcard-no">{{ row.roleName }}</span>
          <t-tag v-if="row.isBuiltin === 1" size="small" variant="light" theme="primary">内置</t-tag>
        </div>
        <div class="mcard-grid">
          ...
          <div v-if="row.remark" class="mcard-cell">
            <div class="k">说明</div>
            <div class="v">{{ row.remark }}</div>
          </div>
        </div>
        <div class="mcard-ft">
    ...
    <t-table v-else :data="list" :columns="columns" row-key="roleId" :loading="loading">
    ...
    >
      <t-form :data="form" label-width="90px">
        <t-form-item label="角色名称" name="roleName">
          <t-input v-model="form.roleName" placeholder="如 值班经理" />
        </t-form-item>
        <t-form-item label="角色标识" name="roleKey">
          <t-input
        ...
        <t-form-item label="排序" name="sortOrder">
          <t-input-number v-model="form.sortOrder" :min="0" style="width: 140px" />
        </t-form-item>
        <t-form-item label="备注" name="remark">
          <t-input v-model="form.remark" placeholder="选填，说明该角色的使用场景" />
        </t-form-item>
      </t-form>
      ...
      <div v-if="loadingCatalog" class="muted" style="padding: 12px 0">权限目录加载中…</div>
      <div v-else class="perm-body" :class="{ readonly: isAdminRole }">
        <div v-for="g in groups" :key="g.key" class="perm-group">
          ...
          <div class="perm-items">
            <t-checkbox
              ...
              :disabled="isAdminRole"
              @change="(v) => togglePerm(p.code, v)"
            >
              {{ p.name }}
              <span v-if="impliedLabel(p.code)" class="perm-imply">（{{ impliedLabel(p.code) }}）</span>
            </t-checkbox>
          </div>
        </div>
      </div>
...
const form = reactive({ roleId: null, roleKey: '', roleName: '', sortOrder: 0, remark: '', permList: [] })
...
function openAdd() {
  Object.assign(form, {
    roleId: null, roleKey: '', roleName: '',
    sortOrder: list.value.length + 1, remark: '', permList: []
  })
...
function openEdit(row) {
  Object.assign(form, {
    ...
    sortOrder: row.sortOrder || 0, remark: row.remark || '',
