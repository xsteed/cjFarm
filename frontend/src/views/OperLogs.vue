  ...
  <div class="page-card">
    <div class="toolbar">
      <span class="tip">
        ...
        免单、挂账核销、退款、改单与权限变更还额外记了金额与原因，供事后对账追责。
        密码与支付密钥等敏感字段一律不入库。
      </span>
      <t-button v-if="canManage" theme="danger" variant="outline" @click="onClean">清理过期日志</t-button>
    </div>

    <div class="filters">
      <t-input v-model="query.operator" placeholder="操作人" style="width: 130px" @enter="reload" />
      <t-select v-model="query.module" :options="moduleOptions" clearable placeholder="全部模块" style="width: 140px" @change="reload" />
      <t-select v-model="query.businessType" :options="typeOptions" clearable placeholder="全部类型" style="width: 120px" @change="reload" />
      <t-select v-model="query.status" :options="statusOptions" clearable placeholder="全部结果" style="width: 120px" @change="reload" />
      <t-input v-model="query.targetId" placeholder="对象ID / 订单号" style="width: 160px" @enter="reload" />
      <t-date-range-picker
        v-model="dateRange"
        clearable
        enable-time-picker
    ...
    <div v-if="isMobile" class="m-list">
      <div v-if="!list.length" class="m-empty">{{ loading ? '加载中…' : '暂无操作记录' }}</div>
      <div v-for="row in list" :key="row.logId" class="mcard">
        <div class="mcard-hd">
          <div style="min-width: 0">
            <div class="mcard-no">{{ row.action }}</div>
            <div class="mcard-sub">{{ row.module }} · {{ row.createTime }}</div>
          </div>
          <t-tag :theme="OPER_STATUS[row.status]?.theme" variant="light">
        ...
        <div class="mcard-grid">
          <div class="mcard-cell">
            <div class="k">操作人</div>
            <div class="v">{{ row.operator || '—' }}<span class="tip" v-if="row.operatorRole"> · {{ row.operatorRole }}</span></div>
          </div>
          <div class="mcard-cell">
            <div class="k">类型</div>
            <div class="v">{{ OPER_TYPE[row.businessType]?.label || row.businessType }}</div>
          </div>
          <div class="mcard-cell">
            <div class="k">对象</div>
        ...
        <div v-if="row.status === 0 && row.errorMsg" class="mcard-sub err">
          失败原因：{{ row.errorMsg }}
        </div>
      </div>
      <div v-if="total > query.pageSize" class="m-pager">
        <t-pagination v-model="query.pageNum" :total="total" :page-size="query.pageSize" size="small" @change="load" />
      </div>
    </div>

    <template v-else>
      <t-table :data="list" :columns="columns" row-key="logId" :loading="loading">
        <template #action="{ row }">
          <div class="strong">{{ row.action }}</div>
          <div class="tip">{{ row.module }}</div>
        </template>
        <template #businessType="{ row }">
          <t-tag :theme="OPER_TYPE[row.businessType]?.theme || 'default'" variant="light">
            {{ OPER_TYPE[row.businessType]?.label || row.businessType }}
          </t-tag>
        </template>
        <template #operator="{ row }">
          <div>{{ row.operator || '—' }}</div>
          <div class="tip">{{ row.operatorRole || '' }}</div>
        </template>
        <template #target="{ row }">
          <span class="tip mono">{{ row.targetType || '—' }}{{ row.targetId ? ' #' + row.targetId : '' }}</span>
        </template>
        <template #detail="{ row }">
          <span class="tip ell" :title="row.detail || row.errorMsg">{{ row.detail || '—' }}</span>
          <div v-if="row.status === 0 && row.errorMsg" class="err ell" :title="row.errorMsg">{{ row.errorMsg }}</div>
        </template>
        <template #status="{ row }">
          <t-tag :theme="OPER_STATUS[row.status]?.theme" variant="light">
            {{ OPER_STATUS[row.status]?.label }}
        ...
        <template #param="{ row }">
          <t-button v-if="row.operParam" theme="primary" variant="text" size="small" @click="onViewParam(row)">
            查看
          </t-button>
    ...
    <!-- 请求参数(脱敏后) -->
    <t-dialog v-model:visible="paramVisible" header="请求参数" :footer="false" width="640px">
...
const MODULES = [
  '登录账号', '系统安全', '员工管理', '角色权限', '桌台管理', '分类管理', '菜品管理',
  '备注管理', '打印机管理', '打印记录', '系统配置', '订单管理', '挂账管理', '退款管理', '操作日志'
]

const query = reactive({
  operator: '', module: '', businessType: '', status: '', targetId: '',
...
function resetQuery() {
  ...
  Object.assign(query, {
    operator: '', module: '', businessType: '', status: '', targetId: '',
    beginTime: '', endTime: '', pageNum: 1
  })
  load()
}

function onViewParam(row) {
