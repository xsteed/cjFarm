  ...
  <div class="page-card">
    <div class="toolbar">
      <div class="toolbar-left">
        <t-input v-model="query.dishName" placeholder="搜索菜品名称" clearable style="width: 220px" @enter="load">
          <template #prefix-icon><search-icon /></template>
        </t-input>
        <t-select v-model="query.categoryId" placeholder="全部分类" clearable style="width: 170px">
          <t-option v-for="c in categories" :key="c.categoryId" :value="String(c.categoryId)" :label="c.categoryName" />
        </t-select>
    ...
    <!-- 网格卡片 -->
    <div v-if="!loading && list.length" class="dish-grid">
      <div v-for="d in list" :key="d.dishId" class="dish-card">
        <div class="pic">
          <img v-if="d.dishImage" :src="d.dishImage" :alt="d.dishName">
          <div v-else class="pic-ph">无图</div>
          <div class="flag" :class="{ off: d.status !== 1 }">{{ d.status === 1 ? '上架中' : '已下架' }}</div>
        </div>
        <div class="info">
          <div class="n">{{ d.dishName }}</div>
          <div class="p"><i>¥</i>{{ minPrice(d) }}<small v-if="d.specs && d.specs.length > 1"> 起</small></div>
          <div class="meta">{{ d.categoryName || '未分类' }}</div>
          <div v-if="canEdit" class="ops">
            <span class="edit" @click="openEdit(d)">编辑</span>
            <span class="del" @click="onDelete(d)">删除</span>
          </div>
        </div>
      </div>
    </div>
    <div v-else-if="!loading && !list.length" class="empty">暂无菜品</div>
    ...
    <t-dialog v-model:visible="dialogVisible" :header="form.dishId ? '编辑菜品' : '新增菜品'" width="640px" :confirm-btn="{ content: '保存', theme: 'primary' }" @confirm="save">
      <t-form :data="form" label-width="90px">
        <t-form-item label="所属分类" name="categoryId">
          <t-select v-model="form.categoryId" placeholder="请选择分类">
            <t-option v-for="c in categories" :key="c.categoryId" :value="c.categoryId" :label="c.categoryName" />
          </t-select>
        </t-form-item>
        <t-form-item label="菜品名称" name="dishName">
          <t-input v-model="form.dishName" placeholder="如 凉拌青瓜" />
        </t-form-item>
        <t-form-item label="菜品图片" name="dishImage">
          <t-upload v-model="form.imageFiles" :auto-upload="false" accept="image/*" theme="image" :max="1" />
        </t-form-item>
        <t-form-item label="菜品描述" name="description">
          <t-textarea v-model="form.description" placeholder="如 清爽开胃" :maxlength="100" />
        </t-form-item>
        <t-form-item label="规格价格" name="specs">
          <div class="spec-editor">
            <div v-for="(s, i) in form.specs" :key="i" class="spec-row">
              <t-input v-model="s.specName" placeholder="规格名，如 份/小份" style="width: 160px" />
              <t-input-number v-model="s.price" :min="0" :decimal-places="2" placeholder="价格" style="width: 130px" />
              <t-button theme="danger" variant="text" @click="removeSpec(i)">删除</t-button>
            </div>
            <t-button variant="dashed" block @click="addSpec">
              <template #icon><add-icon /></template>
              添加规格
            </t-button>
          </div>
...
const form = reactive({ dishId: null, categoryId: null, dishName: '', dishImage: '', description: '', status: 1, sortOrder: 0, specs: [], imageFiles: [] })
...
async function load() {
  loading.value = true
  try {
    const res = await listDishes({ pageNum: pagination.current, pageSize: pagination.pageSize, dishName: query.dishName, categoryId: query.categoryId })
    list.value = res.rows
