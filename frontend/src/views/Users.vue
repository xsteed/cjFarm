      ...
      >
        <div class="mcard-hd">
          <div style="min-width: 0">
            ...
            <div class="mcard-sub">
              {{ row.username }}<span v-if="row.phone"> · {{ row.phone }}</span>
            </div>
          </div>
          <t-tag
      ...
      >
        ...
        >
          <t-input
            ...
            type="password"
            placeholder="至少 6 位，交给员工后请提醒尽快修改"
          />
        </t-form-item>
        <t-form-item
          label="姓名"
        ...
        >
          ...
          <div
            v-if="rolesLoadFailed"
            class="field-tip warn"
          >
            角色列表加载失败（可能缺少「角色查看」权限），暂无法设置员工角色。
          </div>
          <div
            v-else-if="!roleOptions.length"
            class="field-tip"
          >
        ...
        >
          <t-input
            v-model="form.remark"
            placeholder="选填，如所属班次"
          />
        </t-form-item>
      </t-form>
      <div
        v-if="form.userId && isSelfId(form.userId)"
        class="hint"
      >
...
import { deleteUser, listRoles, listUsers, resetUserPassword, saveUser, toggleUserStatus, updateUser } from '../api';
import { getUsername, hasPerm } from '../utils/perm';
import { useIsMobile } from '../utils/useMobile';
import type { Role, UserInfo } from '../types/entities';

// user:edit 才显示增删改类按钮;user:view 进来只看列表。
// 注意:后端同样会校验,这里的显隐只是避免「点了才被拒」的体验问题。
const canEdit = computed(() => hasPerm('user:edit'));

// 列表行在操作函数里要把 userId 传给删除等接口(参数为 number | string),
// UserInfo 字段全可选,这里收紧为必填。
type UserRow = UserInfo & { userId: number };
...
const query = reactive<UserQuery>({ keyword: '', roleId: undefined, status: undefined, pageNum: 1, pageSize: 10 });
...
function isSelfId(id: number): boolean {
...
async function load(): Promise<void> {
  ...
  try {
    ...
    list.value = (res.items || []) as UserRow[];
...
// ---- 启停用 / 删除 ----
function onToggleStatus(row: UserRow): void {
