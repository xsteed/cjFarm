-- ============================================================================
-- 增量迁移 20260924010000：桌台表补充「点餐稳定码」列
...
-- 适用：早期版本、tb_table 尚无 table_code 列的 SQLite 库。
--       （此前该列仅靠启动时运行时迁移兜底，补此脚本与其余加列惯例一致。）
--
-- ⚠️ 幂等性说明：ALTER TABLE ADD COLUMN 不支持 IF NOT EXISTS，列已存在时会报
--      "duplicate column name"（可忽略）。回填（UPDATE）可安全重复执行。
-- 执行：sqlite3 dining.db < 20260924010000_tb_table_add_table_code.sql
-- ============================================================================

ALTER TABLE tb_table ADD COLUMN table_code TEXT DEFAULT '';
