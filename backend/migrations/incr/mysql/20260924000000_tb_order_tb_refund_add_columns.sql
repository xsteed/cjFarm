-- ============================================================================
-- 增量迁移 20260924000000：订单表「结账前状态快照」+ 退款表「重复支付退回标记」列（MySQL）
...
-- 适用：早期版本、上述两表尚无对应列的 MySQL 库。
--
-- ⚠️ 幂等性说明：MySQL 不支持 ADD COLUMN IF NOT EXISTS（MariaDB 支持）。
--      若某列已存在会报 "Duplicate column name"，该报错可忽略；
--      执行前可用 SHOW COLUMNS FROM tb_order / tb_refund; 自查缺哪些列。
-- 执行：mysql -u dining -p dining < 20260924000000_tb_order_tb_refund_add_columns.sql
-- ============================================================================

SET NAMES utf8mb4;

-- 结账前状态快照：0=历史数据未记录(撤销结算时回退为 3)
ALTER TABLE tb_order ADD COLUMN pre_settle_status INT DEFAULT 0;

-- 重复支付自动退回标记：0=普通退款 / 1=重复支付退回
ALTER TABLE tb_refund ADD COLUMN is_duplicate INT DEFAULT 0;
