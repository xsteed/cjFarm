...
func TestInsertPaymentUniqueIndex(t *testing.T) {
	Init(filepath.Join(t.TempDir(), "pay.db"))
	defer DB.Close()
	...
	var n int
	if err := DB.QueryRow(`SELECT COUNT(*) FROM tb_payment`).Scan(&n); err != nil || n != 3 {
	...
	if err := DB.QueryRow(`SELECT COUNT(*) FROM tb_payment WHERE order_no='A'`).Scan(&n); err != nil || n != 1 {
		t.Fatalf("订单A流水条数=%d 期望 1 (err=%v)", n, err)
	}
	var amount int64
	if err := DB.QueryRow(`SELECT amount FROM tb_payment WHERE order_no='A'`).Scan(&amount); err != nil || amount != 1500 {
...
func TestRefundAfterPartialRefund(t *testing.T) {
	Init(filepath.Join(t.TempDir(), "pay2.db"))
	defer DB.Close()
	...
	if _, err := DB.Exec(`UPDATE tb_payment SET status=? WHERE payment_id=?`, PayStatusRefunded, paymentID); err != nil {
		t.Fatalf("模拟部分退款后状态失败: %v", err)
	}

	// 退款处理器使用的查询条件:已支付或已退款
	var id int
	if err := DB.QueryRow(`SELECT payment_id FROM tb_payment WHERE order_no=? AND status IN (?,?) ORDER BY payment_id DESC LIMIT 1`,
