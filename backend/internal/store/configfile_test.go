...
var configEnvKeys = []string{
	...
	"CONFIG_MASTER_KEY", "MASTER_KEY_PATH", "TOKEN_TTL_HOURS", "HARDEN_FILE_ACL",
...
// TestConfigFileEnvPairs 验证完整 config.yaml 到环境变量键名的映射。
func TestConfigFileEnvPairs(t *testing.T) {
	isolateConfigEnv(t)
...
// TestConfigFileSkipsEmptyAndPartial 验证「未配置 / 空串」都不会污染环境。
func TestConfigFileSkipsEmptyAndPartial(t *testing.T) {
	isolateConfigEnv(t)
...
// TestConfigFileUnknownKey 验证键名写错时直接报错而非静默忽略。
...
// TestConfigFileMissing 验证文件不存在时静默跳过(单机部署无需该文件)。
