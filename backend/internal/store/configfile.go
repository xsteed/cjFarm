...
//     store.Getenv(key, def),因此新增来源不需要改动任何一行业务逻辑;
...
// ConfigFile 是 config.yaml 的完整结构。
//
...
type SQLiteSection struct {
	Path *string `yaml:"path"` // -> DB_PATH,默认 ./dining.db
}
...
// configFileName 默认配置文件名(CONFIG_FILE 可覆盖)。
const configFileName = "config.yaml"

// ConfigStats 描述配置加载结果,供启动日志与测试使用。
...
// LoadConfigFile 读取 config.yaml 并返回「环境变量键值对」,不写入进程环境。
//
...
// configFilePath 返回 config.yaml 路径:CONFIG_FILE 可覆盖。
...
//
// 命名:store.LoadConfig 已被「读取 tb_config 业务配置」占用,故加 Deploy
// 前缀区分 —— 一个是数据库里的业务参数,一个是进程启动的部署参数。
//
// 优先级(高 → 低):
//
...
func LoadDeployConfig() (ConfigStats, error) {
	var st ConfigStats

	// 第 2、3 步先各自解析成表,暂不写入 —— 否则先落地的来源会误判成
	// 「已存在的真实环境变量」,把自己的优先级抬高。
	yamlPairs, err := LoadConfigFile()
	if err != nil {
		return st, err
	}
	...
	if err != nil {
		...
		logConfigf("读取 .env 失败,已跳过: %v", err)
	}

	st.YAMLPath, st.YAMLFound = configFilePath(), yamlPairs != nil
...
func (c *ConfigFile) envPairs() map[string]string {
	m := make(map[string]string)

	setStr(m, "PORT", intToStr(c.Server.Port))
	setStrPtr(m, "UPLOAD_DIR", c.Server.UploadDir)
	setStrPtr(m, "STATIC_DIR", c.Server.StaticDir)
	setList(m, "CORS_ORIGINS", c.Server.CorsOrigins)
	setList(m, "TRUSTED_PROXIES", c.Server.TrustedProxies)

	setStrPtr(m, "DB_DRIVER", c.Database.Driver)
	setStrPtr(m, "DB_PATH", c.Database.SQLite.Path)
	setStrPtr(m, "DB_DSN", c.Database.MySQL.DSN)
	setStrPtr(m, "DB_HOST", c.Database.MySQL.Host)
	setStr(m, "DB_PORT", intToStr(c.Database.MySQL.Port))
	setStrPtr(m, "DB_USER", c.Database.MySQL.User)
	setStrPtr(m, "DB_PASSWORD", c.Database.MySQL.Password)
	setStrPtr(m, "DB_NAME", c.Database.MySQL.Name)
	setStrPtr(m, "DB_PARAMS", c.Database.MySQL.Params)

	setStrPtr(m, "CONFIG_MASTER_KEY", c.Security.MasterKey)
	setStrPtr(m, "MASTER_KEY_PATH", c.Security.MasterKeyPath)
	setStr(m, "TOKEN_TTL_HOURS", intToStr(c.Security.TokenTTLHours))
	if c.Security.HardenFileACL != nil {
		m["HARDEN_FILE_ACL"] = boolFlag(*c.Security.HardenFileACL)
	}

	setStrPtr(m, "ADMIN_USER", c.Admin.User)
	setStrPtr(m, "ADMIN_PASS", c.Admin.Pass)
...
// logConfigf 是 logger.Infof 的薄包装,便于本文件与日志格式对齐。
func logConfigf(format string, args ...interface{}) {
	logger.Infof("[config] "+format, args...)
}
