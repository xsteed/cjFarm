...
func InsertOperLog(l po.OperLog) error {
	_, err := store.DB.Exec(`INSERT INTO tb_oper_log(module, business_type, action, method, request_url,
		operator_id, operator, operator_role, oper_ip, target_type, target_id,
		oper_param, detail, status, error_msg, cost_ms, create_time)
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
func ListOperLogs(q OperLogQuery, pageNum, pageSize int) (int, []po.OperLog, error) {
	where := []string{"1=1"}
	args := []interface{}{}
	...
	var total int
	if err := store.DB.QueryRow(`SELECT COUNT(*) FROM tb_oper_log`+cond, args...).Scan(&total); err != nil {
		return 0, nil, err
	}
	rows, err := store.DB.Query(`SELECT `+OperLogCols+` FROM tb_oper_log`+cond+
	...
	list := []po.OperLog{}
	for rows.Next() {
		l, scanErr := ScanOperLog(rows)
		if scanErr != nil {
			return 0, nil, scanErr // 审计行扫描失败必须报错,静默丢弃等于抹掉留痕
		}
		list = append(list, l)
	}
...
func CleanOperLogs(beforeTime string) (int64, error) {
	res, err := store.DB.Exec(`DELETE FROM tb_oper_log WHERE create_time < ?`, beforeTime)
	if err != nil {
		return 0, err
	}
...
func CleanExpiredOperLogs() int64 {
	...
	n, err := CleanOperLogs(time.Now().AddDate(0, 0, -days).Format(conf.TimeLayout))
	if err != nil {
...
func AuditEnabled() bool {
	v := strings.TrimSpace(strings.ToLower(infra.Getenv(conf.EnvAuditLogEnabled, conf.DefaultAuditLogEnabled)))
...
func AuditLogGet() bool {
	v := strings.TrimSpace(strings.ToLower(infra.Getenv(conf.EnvAuditLogGet, conf.DefaultAuditLogGet)))
	return v == "1" || v == "true" || v == "on"
}

// AuditRetentionDays 操作日志保留天数(AUDIT_RETENTION_DAYS,默认 365,0 表示不清理)。
func AuditRetentionDays() int {
	n, err := strconv.Atoi(strings.TrimSpace(infra.Getenv(conf.EnvAuditRetentionDays, conf.DefaultAuditRetentionDays)))
	if err != nil || n < 0 {
		return 365
	}
	return n
...
//
// 名单里刻意保留 "key" 这个极短词:打印机绑定请求体 {"sn":..., "key":...}
// 里的识别码 KEY 配合 SN 就能把打印机绑到攻击者账号,明文落库等同泄密。
// 子串匹配意味着 monkey/whiskey/roleKey 等无关键也会被一并脱敏 —— 这是
// 刻意的取舍:审计脱敏宁可多脱不可漏脱,少记一点信息总好过泄露凭据。
// SN 是设备编号而非凭据,保持明文,便于排查「绑的是哪台设备」。
var sensitiveKeyParts = []string{
	"password", "passwd", "pwd",
	"secret", "token", "key", "ukey", "apiv3",
