...
// key 是极短词,子串匹配会顺带把 roleKey 也脱敏 —— 这是「宁多脱不漏脱」的取舍。
func TestMaskParamsHidesSecrets(t *testing.T) {
	raw := `{"username":"admin","password":"p@ss","oldPassword":"oldPwd","newPassword":"newPwd",
		"wxpay_apiv3_key":"deadbeef","feie_ukey":"ukey-123","key":"feie-key-456",
		"wxpay_private_key_path":"/data/key.pem","roleKey":"manager","shop_name":"老王烧烤",
		"items":[{"dishId":1,"remark":"不要辣"}]}`
	...
	for _, secret := range []string{"p@ss", "oldPwd", "newPwd", "deadbeef", "ukey-123", "feie-key-456"} {
	...
	for _, keep := range []string{"admin", "老王烧烤", "不要辣", "/data/key.pem"} {
	...
	if decoded["key"] != "******" {
		t.Errorf("key 应被替换为 ******,实际 %v", decoded["key"])
	}
	if decoded["roleKey"] != "******" {
		t.Errorf("roleKey 含 key 子串,按「宁多脱不漏脱」应一并脱敏,实际 %v", decoded["roleKey"])
	}
}
