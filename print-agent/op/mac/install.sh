...
echo ""
echo "== 下一步(macOS 15 及以上请务必一看) =="
echo "代理已部署为 App:  $APP"
echo "配置文件/日志在:   $DATA_DIR"
echo ""
echo "如果打印时报「无法连接打印机 ... no route to host」,而您在本目录手工执行"
echo "  ./print-agent-darwin-arm64 --probe <打印机IP>   能显示「可达」,"
echo "那说明是系统的「本地网络」隐私权限拦下了开机自启的代理(终端不受该限制)。处置二选一:"
echo "  ① 授权:打开「系统设置 → 隐私与安全性 → 本地网络」,允许「长健农场打印代理」;"
echo "     直接打开该面板:"
echo "       open \"x-apple.systempreferences:com.apple.preference.security?Privacy_LocalNetwork\""
echo "  ② 改走系统打印服务(不受该权限影响):先在「系统设置 → 打印机与扫描仪」以 IP 方式"
echo "     添加本店打印机,再到 $DATA_DIR/agent.env 里加一行"
echo "       PRINT_AGENT_PRINT_VIA=auto"
echo "     然后执行 ./start.sh 让配置生效。"
echo "查看状态与最近日志: ./p.sh"
